...
/**
* Walk the directory tree to locate the nag-doc.json file.
...
var getProjectData = function(dir) {
    ...
    while (dirs.length && !projectData) {
        if (dirCount++ > 5000) {
            Y.log('Scanned ' + dirCount + ' directories looking for a nag-doc.json file, something is probably wrong here..', 'error', 'nag-doc');
            process.exit(1);
        }
        ...
        dirs.forEach(function(dir) {
            ...
            try {
                ...
                names.forEach(function(name) {
                    ...
                    if (Y.Files.isFile(p)) {
                        projectData = projectData || getFileData(p, name, 'nag-doc.json');
                        // 'package.json' is used for auxilliary configuration
                        // if it's found. Formerly, it was only found if it
                        // came _before_'nag-doc.json' in the folder tree
                        // (never in the same folder).
                        // This code will find 'package.json' in the same
                        // folder as 'nag-doc.json'.
                        // If there is no 'nag-doc.json', former algorithm would
                    ...
                    if (Y.Files.isDirectory(p)) {
                        ...
                        if (name === 'node_modules') {
                            Y.log('Skipping node_modules directory while scanning for nag-doc.json', 'warn', 'nag-doc');
                            return;
                        }
                        childDirs.push(p);
                    }
                ...
                Y.log('Accessing dir (' + dir + ') threw an error', 'warn', 'nag-doc');
            }
        });
...
var getDirs = function(dir) {
    ...
    dirs.forEach(function(d) {
        try {
          ...
          //prevent symlinks from killing process
        }
    });
...
var validatePaths = function(paths, ignore) {
    ...
    paths.forEach(function(path) {
        ...
        glob_paths.forEach(function(dir) {
            ...
            //need to remove ../ and ./ in order to makr sure glob match will work
            if (minimatch(dir, glob.replace('../', '').replace('./', ''), { period: true })) {
                newpaths.push(dir);
                is_globbed = true;
            }
        });
