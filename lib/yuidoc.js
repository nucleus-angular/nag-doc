...
/**
This is the __module__ description for the `Nag Doc` module.
    ...
    Y.YUIDoc.prototype = {
        ...
        walk: function() {
            Y.each(this.options.paths, function(dir) {
                this.rootParsingDirectory = dir;
                this.parsedir(dir);
            }, this);
        },
        rootParsingDirectory: null,
        /**
        ...
        parsedir: function(dir) {
            ...
            //need to remove ../ and ./ in order to makr sure glob match will work with ignorePaths
            var relativePathToRoot = dir.replace(this.rootParsingDirectory.replace('../', '').replace('./', ''), '');

            if(relativePathToRoot.length > 0 && relativePathToRoot.substr(0, 1) === '/') {
                relativePathToRoot = relativePathToRoot.substr(1);
            }

            var globMatch = false;
            var globMatchContent = null;
            var runCurrentDirectory = false;

            if(!Y.Lang.isArray(this.options.ignorePaths)) {
                runCurrentDirectory = true;
            } else if(this.options.ignorePaths.indexOf(relativePathToRoot) === -1) {
                globMatch = false;
                globMatchContent = null;
                Y.each(this.options.ignorePaths, function(glob) {
                    if(globMatch === false && minimatch(relativePathToRoot, glob, {period: true})) {
                        globMatch = true;
                        globMatchContent = glob;
                    }
                });
                ...
                if(globMatch === false) {
                    ...
                    Y.log('skipping directory : ' + relativePathToRoot + ' : based on glob match : ' + globMatchContent, 'info', 'nag-doc');
                }
            }

            if(runCurrentDirectory === true) {
                ...
                if (ext) {
                    if (ext in self.options.extensions) {
                        ...
                        if (Y.Files.exists(fullpath)) {
                            ...
                            Y.log('File skipped: ' + fullpath, 'warn', 'nag-doc');
                        }
                    }
                }
            });
        ...
        getSelleck: function(fullpath) {
            ...
            if (self.options.selleck) {
                ...
                //Y.log('Checking for Selleck data: ' + comp, 'info', 'nag-doc');
                if (Y.Files.exists(comp)) {
                    try {
                        ...
                        Y.log('JSON parse failed on Selleck component.json file: ' + comp, 'error', 'nag-doc');
                    }
                }
            }
        },
        ...
        writeJSON: function(parser) {
            ...
            if (self.options.writeJSON) {
                ...
                //no need to delete the output folder
                /*if (Y.Files.exists(self.options.outdir) && !self.options.nodeleteout) {
                    Y.log('Found out dir, deleting: ' + self.options.outdir, 'warn', 'nag-doc');
                    rimraf.sync(self.options.outdir);
                }*/
                if (!Y.Files.exists(self.options.outdir)) {
                    Y.log('Making out dir: ' + self.options.outdir, 'info', 'nag-doc');
                    try {
                        fs.mkdirSync(self.options.outdir, 0777);
                    } catch (e) {
                        Y.log('Outdir creation failed', 'warn', 'nag-doc');
                    }
                }
        ...
        lint: function(warnings) {
            ...
            if (warnings && warnings.length) {
                code = 1;
                console.log('NAg Doc found', warnings.length, 'lint errors in your docs');
        ...
        run: function() {
            /**
             * Timestamp holder so we know when Nag Doc started the parse process.
             * @property starttime
             * @type Timestamp
             */
            Y.log('Nag Doc Starting from: ' + this.options.paths.join(','), 'info', 'nag-doc');
            ...
            /**
             * Timestamp holder so we know when Nag Doc has finished the parse process.
             * @property endtime
             * @type Timestamp
             */
            this.endtime = new Date().getTime();
            Y.log('Parsed ' + this.filecount + ' files in ' + ((this.endtime - this.starttime) / 1000) + ' seconds', 'info', 'nag-doc');
