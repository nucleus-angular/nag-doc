  ...
  Y.DocBuilder.prototype = {
    ...
    augmentTopLevelItem: function(type, topLevelObject) {
      ...
      self.searchableData.push({
        ...
        jsonPath: self.generateJsonFilePath(type, self.getFileNameFromTopLevelObject(topLevelObject), true),
        componentType: topLevelItemSingleName,
        search: searchValue
      });
    },

    getFileNameFromTopLevelObject: function(topLevelObject) {
      var fileName = topLevelObject.name;

      if(topLevelObject.module) {
        fileName += '.' + topLevelObject.module;
      }
    ...
    buildJsonFile: function(type, jsonObject, fileName) {
      var self = this;
      var fileName = fileName || self.getFileNameFromTopLevelObject(jsonObject);
