...
YUI.add('docparser', function(Y) {
    ...
    fixType = function(string){ return string;},
    /**
    ...
    */
    implodeString = function(str) {
        return str.replace(REGEX_GLOBAL_LINES, '!~NAGDOC_LINE~!');
    },
    /*
    ...
    */
    explodeString = function(str) {
        return str.replace(/!~NAGDOC_LINE~!/g, '\n');
    },
    ...
    CURRENT_NGSERVICE = 'currentngservice',
    CURRENT_NGDIRECTIVE = 'currentngdirective',
    CURRENT_NGCONTROLLER = 'currentngcontroller',
    CURRENT_NGFILTER = 'currentngfilter',
    CURRENT_NGVALUE = 'currentngvalue',
    ...
    DIGESTERS = {
        ...
        'param': function(tagname, value, target, block) {
            ...
            var groupname = (tagname === 'param' ? 'params' : 'nghtmlattributes');
            ...
            if (name.indexOf('[') > -1) {
                ...
                if (match) {
                    ...
                    if (parts.length > 1) {
                        ...
                        //hack: the current parseing for params does not allow to do [param=[1,2,3]]
                        if(optdefault.substr(0, 1) == '[') {
                            optdefault += ']';
                        }
                    }
                }
            ...
            if (name.indexOf('.') > -1) {
                match = name.split('.');
                parent = trim(match[0]);
                Y.each(target[groupname], function(param) {
        ...
        'module': function(tagname, value, target, block) {
            ...
            //only process the module element is #module is the only main tag on the docblock
            Y.each(block, function(tag, key) {
                if(go === true && mainBlockTypes.indexOf(tag.tag) !== -1) {
                    go = false;
                }
            });
      ...
      // A key bock type for declaring classes, subsequent
      // member blocks will be assigned to this class
      'ngservice': function(tagname, value, target, block) {
        var _namespace, _value = value;
        ...
        this.set(CURRENT_NGSERVICE, value);
        var fullname = this.get(CURRENT_NGSERVICE);
        var host = this.ngservices[fullname],
        parent = this.get(CURRENT_MODULE);
      ...
      // A key bock type for declaring classes, subsequent
      // member blocks will be assigned to this class
      'ngdirective': function(tagname, value, target, block) {
        var _namespace, _value = value;
        ...
        this.set(CURRENT_NGDIRECTIVE, value);
            var fullname = this.get(CURRENT_NGDIRECTIVE);
        var host = this.ngdirectives[fullname],
        parent = this.get(CURRENT_MODULE);
        ...
        //before it was defined in a "class" tag
        host = Y.merge(host, target);
        this.ngdirectives[fullname] = host;
        parent = this.get(CURRENT_SUBMODULE);
        if (parent) {
          //this.set(CURRENT_SUBMODULE, parent);
          host.submodule = parent;
      ...
      // A key bock type for declaring classes, subsequent
      // member blocks will be assigned to this class
      'ngcontroller': function(tagname, value, target, block) {
        var _namespace, _value = value;
        ...
        this.set(CURRENT_NGCONTROLLER, value);
        var fullname = this.get(CURRENT_NGCONTROLLER);
        var host = this.ngcontrollers[fullname],
        parent = this.get(CURRENT_MODULE);
        ...
        //before it was defined in a "class" tag
        host = Y.merge(host, target);
        this.ngcontrollers[fullname] = host;
        parent = this.get(CURRENT_SUBMODULE);
        if (parent) {
          //this.set(CURRENT_SUBMODULE, parent);
          host.submodule = parent;
      ...
      // A key bock type for declaring classes, subsequent
      // member blocks will be assigned to this class
      'ngfilter': function(tagname, value, target, block) {
        var _namespace, _value = value;
        ...
        this.set(CURRENT_NGFILTER, value);
        var fullname = this.get(CURRENT_NGFILTER);
        var host = this.ngfilters[fullname],
        parent = this.get(CURRENT_MODULE);
      ...
      // A key bock type for declaring classes, subsequent
      // member blocks will be assigned to this class
      'ngvalue': function(tagname, value, target, block) {
        var _namespace, _value = value;
        ...
        this.set(CURRENT_NGVALUE, value);
        var fullname = this.get(CURRENT_NGVALUE);
        var host = this.ngvalues[fullname],
        parent = this.get(CURRENT_MODULE);
        ...
        'event': 'property',
        'respondto': 'property',
        'nghtmlattribute': 'param',
        'ngwatch': 'property',

        'ngdirectivecontroller': function(tagname, value, target, block) {
          target.ngtype = 'controller';
        },
        'ngscope': function(tagname, value, target, block) {
          target.ngtype = 'scope';
        },
        ...
        'example': function(tagname, value, target, block) {
            if (!Lang.isArray(target['example'])) {
                target['example'] = [];
            }
    ...
    DocParser.ATTRS = {
      ...
      currentngservice: {
        setter: function(val) {
          ...
          this.set('lastngservice', this.get(CURRENT_NGSERVICE));
          val = trim(val);
          if (!(val in this.ngservices)) {
            var ns = this.get(CURRENT_NAMESPACE),
            name = (ns && ns !== '' && (val.indexOf(ns + '.') !== 0)) ? ns + '.' + val : val,
            ngservice = this.ngservices[name] = {
              name: name,
              shortname: val,
              ngserviceitems: [],
              plugins: [],
            ...
            ngservice.module = this.get(CURRENT_MODULE);
            if (this.get(CURRENT_SUBMODULE)) {
              ngservice.submodule = this.get(CURRENT_SUBMODULE);
            }
            ngservice.namespace = ns;
          }
          return name;
        }
      },
      ...
      currentngdirective: {
        setter: function(val) {
          ...
          this.set('lastngservice', this.get(CURRENT_NGDIRECTIVE));
          val = trim(val);
          if (!(val in this.ngdirectives)) {
            var ns = this.get(CURRENT_NAMESPACE),
            name = (ns && ns !== '' && (val.indexOf(ns + '.') !== 0)) ? ns + '.' + val : val,
            ngdirective = this.ngdirectives[name] = {
              name: name,
              shortname: val,
              ngdirectiveitems: [],
              plugins: [],
            ...
            ngdirective.module = this.get(CURRENT_MODULE);
            if (this.get(CURRENT_SUBMODULE)) {
              ngdirective.submodule = this.get(CURRENT_SUBMODULE);
            }
            ngdirective.namespace = ns;
          }
          return name;
        }
      },
      ...
      currentngcontroller: {
        setter: function(val) {
          ...
          this.set('lastngservice', this.get(CURRENT_NGCONTROLLER));
          val = trim(val);
          if (!(val in this.ngcontrollers)) {
            var ns = this.get(CURRENT_NAMESPACE),
            name = (ns && ns !== '' && (val.indexOf(ns + '.') !== 0)) ? ns + '.' + val : val,
            ngcontroller = this.ngcontrollers[name] = {
              name: name,
              shortname: val,
              ngcontrolleritems: [],
              plugins: [],
            ...
            ngcontroller.module = this.get(CURRENT_MODULE);
            if (this.get(CURRENT_SUBMODULE)) {
              ngcontroller.submodule = this.get(CURRENT_SUBMODULE);
            }
            ngcontroller.namespace = ns;
          }
          return name;
        }
      },
      ...
      currentngfilter: {
        setter: function(val) {
          ...
          this.set('lastngservice', this.get(CURRENT_NGFILTER));
          val = trim(val);
          if (!(val in this.ngfilters)) {
            var ns = this.get(CURRENT_NAMESPACE),
            name = (ns && ns !== '' && (val.indexOf(ns + '.') !== 0)) ? ns + '.' + val : val,
            ngfilter = this.ngfilters[name] = {
              name: name,
              shortname: val,
              plugins: [],
              extensions: [],
            ...
            ngfilter.module = this.get(CURRENT_MODULE);
            if (this.get(CURRENT_SUBMODULE)) {
              ngfilter.submodule = this.get(CURRENT_SUBMODULE);
            }
            ngfilter.namespace = ns;
          }
          return name;
        }
      },
      ...
      currentngvalue: {
        setter: function(val) {
          ...
          this.set('lastngservice', this.get(CURRENT_NGVALUE));
          val = trim(val);
          if (!(val in this.ngvalues)) {
            var ns = this.get(CURRENT_NAMESPACE),
            name = (ns && ns !== '' && (val.indexOf(ns + '.') !== 0)) ? ns + '.' + val : val,
            ngvalue = this.ngvalues[name] = {
              name: name,
              shortname: val,
              plugins: [],
              extensions: [],
            ...
            ngvalue.module = this.get(CURRENT_MODULE);
            if (this.get(CURRENT_SUBMODULE)) {
              ngvalue.submodule = this.get(CURRENT_SUBMODULE);
            }
            ngvalue.namespace = ns;
          }
          return name;
        }
      }
    ...
    Y.extend(DocParser, Y.Base, {
        ...
        */
        ...
        clearTopLevelItems: function(skip) {
            if(skip != 'CURRENT_SUBMODULE') {
                this.set(CURRENT_SUBMODULE, '');
            }
            if(skip != 'CURRENT_CLASS') {
                this.set(CURRENT_CLASS, '');
            }
            if(skip != 'CURRENT_NGSERVICE') {
                this.set(CURRENT_NGSERVICE, '');
            }
            if(skip != 'CURRENT_NGDIRECTIVE') {
                this.set(CURRENT_NGDIRECTIVE, '');
            }
            if(skip != 'CURRENT_NGCONTROLLER') {
                this.set(CURRENT_NGCONTROLLER, '');
            }
        },
        ...
        initializer: function() {
            ...
            self.after('currentfileChange', function(e) {
                /*
                * File changed, so we reset top level items
                * You should use @for if you want to reference another class
                * in different file.
                */
                self.clearTopLevelItems();
            });
            ...
            self.after('currentsubmoduleChange', function(e) {
                if(e.newVal != '') {
                    self.clearTopLevelItems('CURRENT_SUBMODULE');
                }
                var mod = e.newVal, classes = self.classes,
                    parent;
            ...
            self.after('currentclassChange', function(e) {
                if(e.newVal != '') {
                    self.clearTopLevelItems('CURRENT_CLASS');
                }
                var clazz = e.newVal;
            ...
            self.after('currentngserviceChange', function(e) {
                if(e.newVal != '') {
                    self.clearTopLevelItems('CURRENT_NGSERVICE');
                }
                var ngservice = e.newVal;
                Y.each(self.ngserviceItems, function(item) {
                    if (!(item["ngservice"])) {
                        item["ngservice"] = ngservice;
                    }
                });
                // Y.log(self.ngserviceItems);
            });

            self.after('currentngdirectiveChange', function(e) {
                if(e.newVal != '') {
                    self.clearTopLevelItems('CURRENT_NGDIRECTIVE');
                }
                var ngdirective = e.newVal;
                Y.each(self.ngdirectiveitems, function(item) {
                    if (!(item["ngdirective"])) {
                        item["ngdirective"] = ngdirective;
                    }
                });
                // Y.log(self.ngdirectiveitems);
            });

            self.after('currentngcontrollerChange', function(e) {
                if(e.newVal != '') {
                    self.clearTopLevelItems('CURRENT_NGCONTROLLER');
                }
                var ngcontroller = e.newVal;
                Y.each(self.ngcontrolleritems, function(item) {
                    if (!(item["ngcontroller"])) {
                        item["ngcontroller"] = ngcontroller;
                    }
                });
                // Y.log(self.ngcontrolleritems);
            });
...
// the first token may be the description, otherwise it should be a tag
        ...
        processblock: function(block) {
            ...
            Y.each(block, function(tag) {
                ...
                if (tag && tag.tag) {
                    ...
                    if(digestname.indexOf(':') > -1) {
                        digestname = digestname.split(':')[0];
                    }
                    ...
                    if (digestname in this.digesters) {
                        ...
                        ret = digester.call(this, truedigestname, value, target, block);
                        host = host || ret;
                    } else {
                        target[name] = value;
                    }
            ...
            //todo: this repetitive code can probably be refactored
            if (host) {
                Y.mix(host, target);
            ...
            } else if(this.get(CURRENT_NGSERVICE)) {
              this.ngserviceitems.push(target);
              target['ngservice'] = this.get(CURRENT_NGSERVICE);
              target.module = this.get(CURRENT_MODULE);
              host = this.get(CURRENT_SUBMODULE);
              if (host) {
                target.submodule = host;
            ...
            } else if(this.get(CURRENT_NGDIRECTIVE)) {
              this.ngdirectiveitems.push(target);
              target['ngdirective'] = this.get(CURRENT_NGDIRECTIVE);
              target.module = this.get(CURRENT_MODULE);
              host = this.get(CURRENT_SUBMODULE);
              if (host) {
                target.submodule = host;
            ...
            } else if(this.get(CURRENT_NGCONTROLLER)) {
              this.ngcontrolleritems.push(target);
              target['ngcontroller'] = this.get(CURRENT_NGCONTROLLER);
              target.module = this.get(CURRENT_MODULE);
              host = this.get(CURRENT_SUBMODULE);
              if (host) {
                target.submodule = host;
            ...
            if(this.get(CURRENT_NGDIRECTIVE)) {
              if(target['example']) {
                this.ngdirectives[this.get(CURRENT_NGDIRECTIVE)]['example'] = target['example'];
              }
              if(target['nghtmlattributes']) {
                this.ngdirectives[this.get(CURRENT_NGDIRECTIVE)]['nghtmlattributes'] = target['nghtmlattributes'];
              }
            } else if(this.get(CURRENT_NGFILTER)) {
              var currentNgfilter = this.ngfilters[this.get(CURRENT_NGFILTER)];

              //cp over the valid items
              var ngfilterCopyParameters = [
                'params',
                'return',
                'deprecated',
                'access',
              ...
              ngfilterCopyParameters.forEach(function(parameter) {
                  if(target[parameter]) {
                        currentNgfilter[parameter] = target[parameter];
                  }
              });
            } else if(this.get(CURRENT_NGVALUE)) {
              var currentNgvalue = this.ngvalues[this.get(CURRENT_NGVALUE)];

              //cp over the valid items
              var ngvalueCopyParameters = [
                'deprecated',
                'access',
                'final',
                'static',
              ...
              ngvalueCopyParameters.forEach(function(parameter) {
                if(target[parameter]) {
                  currentNgvalue[parameter] = target[parameter];
                }
              });
            }
        },
        ...
        transform: function(commentmap) {
            var self = this,
                ...
                ngservices = self.ngservices = {},
                ngdirectives = self.ngdirectives = {},
                ngcontrollers = self.ngcontrollers = {},
                ngfilters = self.ngfilters = {},
                ngvalues = self.ngvalues = {},
                classitems = self.classitems = [],
                ngserviceitems = self.ngserviceitems = [],
                ngdirectiveitems = self.ngdirectiveitems = [],
                ngcontrolleritems = self.ngcontrolleritems = [],
                data = self.data = {
                    project: project,
                    files: files,
                    modules: modules,
                    ...
                    ngdirectives: ngdirectives,
                    ngcontrollers: ngcontrollers,
                    ngfilters: ngfilters,
                    ngvalues: ngvalues,
                    classitems: classitems,
                    ngserviceitems: ngserviceitems,
                    ngdirectiveitems: ngdirectiveitems,
                    ngcontrolleritems: ngcontrolleritems
                };
            ...
            Y.each(ngservices, function(ngservices, name) {
              if (ngservices.module) {
                modules[ngservices.module].ngservices[name] = 1;
              }
              //console.error('------------------------------');
              //console.error(clazz);
              //console.error(modules[clazz.submodule]);
              //console.error('------------------------------');
              if (ngservices.submodule) {
                modules[ngservices.submodule].ngservices[name] = 1;
                if (!modules[ngservices.submodule].description) {
                  modules[ngservices.submodule].description = ngservices.description;
                }
              }

              if (ngservices.file) {
                files[ngservices.file].ngservices[name] = 1;
                if (modules[ngservices.module]) {
                  modules[ngservices.module].file = ngservices.file;
                  modules[ngservices.module].line = ngservices.line;
                }
                if (modules[ngservices.submodule]) {
                  modules[ngservices.submodule].file = ngservices.file;
                  modules[ngservices.submodule].line = ngservices.line;
                }
              }
              if (ngservices.uses && ngservices.uses.length) {
                ngservices.uses.forEach(function(u) {
                  var c = classes[u];
                  if (c) {
                    c.extension_for.push(ngservices.name);
                  }
                });
              }
            });

            Y.each(ngdirectives, function(ngdirectives, name) {
              if (ngdirectives.module) {
                modules[ngdirectives.module].ngdirectives[name] = 1;
              }
              //console.error('------------------------------');
              //console.error(clazz);
              //console.error(modules[clazz.submodule]);
              //console.error('------------------------------');
              if (ngdirectives.submodule) {
                modules[ngdirectives.submodule].ngdirectives[name] = 1;
                if (!modules[ngdirectives.submodule].description) {
                  modules[ngdirectives.submodule].description = ngdirectives.description;
                }
              }
  
              if (ngdirectives.file) {
                files[ngdirectives.file].ngdirectives[name] = 1;
                if (modules[ngdirectives.module]) {
                  modules[ngdirectives.module].file = ngdirectives.file;
                  modules[ngdirectives.module].line = ngdirectives.line;
                }
                if (modules[ngdirectives.submodule]) {
                  modules[ngdirectives.submodule].file = ngdirectives.file;
                  modules[ngdirectives.submodule].line = ngdirectives.line;
                }
              }
              if (ngdirectives.uses && ngdirectives.uses.length) {
                ngdirectives.uses.forEach(function(u) {
                  var c = classes[u];
                  if (c) {
                    c.extension_for.push(ngdirectives.name);
                  }
                });
              }
            });

            Y.each(ngcontrollers, function(ngcontrollers, name) {
              if (ngcontrollers.module) {
                modules[ngcontrollers.module].ngcontrollers[name] = 1;
              }
              //console.error('------------------------------');
              //console.error(clazz);
              //console.error(modules[clazz.submodule]);
              //console.error('------------------------------');
              if (ngcontrollers.submodule) {
                modules[ngcontrollers.submodule].ngcontrollers[name] = 1;
                if (!modules[ngcontrollers.submodule].description) {
                  modules[ngcontrollers.submodule].description = ngcontrollers.description;
                }
              }
  
              if (ngcontrollers.file) {
                files[ngcontrollers.file].ngcontrollers[name] = 1;
                if (modules[ngcontrollers.module]) {
                  modules[ngcontrollers.module].file = ngcontrollers.file;
                  modules[ngcontrollers.module].line = ngcontrollers.line;
                }
                if (modules[ngcontrollers.submodule]) {
                  modules[ngcontrollers.submodule].file = ngcontrollers.file;
                  modules[ngcontrollers.submodule].line = ngcontrollers.line;
                }
              }
              if (ngcontrollers.uses && ngcontrollers.uses.length) {
                ngcontrollers.uses.forEach(function(u) {
                  var c = classes[u];
                  if (c) {
                    c.extension_for.push(ngcontrollers.name);
                  }
                });
              }
            });

            Y.each(ngfilters, function(ngfilters, name) {
              if (ngfilters.module) {
                modules[ngfilters.module].ngfilters[name] = 1;
              }
              //console.error('------------------------------');
              //console.error(clazz);
              //console.error(modules[clazz.submodule]);
              //console.error('------------------------------');
              if (ngfilters.submodule) {
                modules[ngfilters.submodule].ngfilters[name] = 1;
                if (!modules[ngfilters.submodule].description) {
                  modules[ngfilters.submodule].description = ngfilters.description;
                }
              }
  
              if (ngfilters.file) {
                files[ngfilters.file].ngfilters[name] = 1;
                if (modules[ngfilters.module]) {
                  modules[ngfilters.module].file = ngfilters.file;
                  modules[ngfilters.module].line = ngfilters.line;
                }
                if (modules[ngfilters.submodule]) {
                  modules[ngfilters.submodule].file = ngfilters.file;
                  modules[ngfilters.submodule].line = ngfilters.line;
                }
              }
              if (ngfilters.uses && ngfilters.uses.length) {
                ngfilters.uses.forEach(function(u) {
                  var c = classes[u];
                  if (c) {
                    c.extension_for.push(ngfilters.name);
                  }
                });
              }
            });

            Y.each(ngvalues, function(ngvalues, name) {
              if (ngvalues.module) {
                modules[ngvalues.module].ngvalues[name] = 1;
              }
              //console.error('------------------------------');
              //console.error(clazz);
              //console.error(modules[clazz.submodule]);
              //console.error('------------------------------');
              if (ngvalues.submodule) {
                modules[ngvalues.submodule].ngvalues[name] = 1;
                if (!modules[ngvalues.submodule].description) {
                  modules[ngvalues.submodule].description = ngvalues.description;
                }
              }
  
              if (ngvalues.file) {
                files[ngvalues.file].ngvalues[name] = 1;
                if (modules[ngvalues.module]) {
                  modules[ngvalues.module].file = ngvalues.file;
                  modules[ngvalues.module].line = ngvalues.line;
                }
                if (modules[ngvalues.submodule]) {
                  modules[ngvalues.submodule].file = ngvalues.file;
                  modules[ngvalues.submodule].line = ngvalues.line;
                }
              }
              if (ngvalues.uses && ngvalues.uses.length) {
                ngvalues.uses.forEach(function(u) {
                  var c = classes[u];
                  if (c) {
                    c.extension_for.push(ngvalues.name);
                  }
                });
              }
            });
            ...
            Y.each(ngserviceitems, function(v) {
              if (!v.itemtype) {
                self.warnings.push({
                  message: 'Missing item type' + (v.description ? '\n' + v.description : ''),
                  line:  stringlog(v)
            ...
            Y.each(ngdirectiveitems, function(v) {
              if (!v.itemtype) {
                self.warnings.push({
                  message: 'Missing item type' + (v.description ? '\n' + v.description : ''),
                  line:  stringlog(v)
            ...
            Y.each(ngcontrolleritems, function(v) {
              if (!v.itemtype) {
                self.warnings.push({
                  message: 'Missing item type' + (v.description ? '\n' + v.description : ''),
                  line:  stringlog(v)
